...
import ru.sfedu.textile.classes.ex4set.Products;
import ru.sfedu.textile.classes.ex4set.Sales;
...
import static ru.sfedu.textile.constants.Constants.NOT_UPDATED;

public class DPHibernateEx4set implements IDPHibernateEx4 {

    private static Logger log = LogManager.getLogger(DPHibernateEx4set.class);
    private Session openSession() {
        return HibernateUtil.getSessionFactory(SCHEMA_EX4_SET).openSession();
    }
    ...
    public void issueSale(String[] articles) {
        try {
            ...
            if (sale.getProducts().size()!=0) {
                Session session = openSession();
                Set<Products> setToRemove = new HashSet<>();
